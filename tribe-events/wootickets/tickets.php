...
?>
    <form id="buy-tickets" action="<?php echo esc_url( wc_get_cart_url() ) ?>" class="<?php echo esc_attr( implode( ' ', $cart_classes ) ); ?>" method="post" enctype='multipart/form-data'>
				...
				if ( 0 !== $available ) {
					...
					if ( $available ) {
						...
						echo sprintf( esc_html__( '%1$s available', 'event-tickets-plus' ),
							'<span class="available-stock" data-product-id="' . esc_attr( $ticket->ID ) . '">' . tribe_tickets_get_readable_amount( $stock, null, false ) . '</span>' ); ?> </span>
                <?php
					}
