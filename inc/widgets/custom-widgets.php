<?php class WP_Widget_Recent_Posts_w_thumbs extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'widget_recent_entries', 'description' => __( "Recent posts with thumbnails") );
        parent::__construct('recent-posts', __('Most shared posts'), $widget_ops);
    ...
    function widget($args, $instance) {
        ...
        $r = new WP_Query( apply_filters( 'widget_posts_args', array( 'posts_per_page' => $number, 'no_found_rows' => true, 'date_query' => array( array( 'column' => 'post_date_gmt', 'after'  => '90 days ago', )), 'meta_key' => '_facebook_likes', 'orderby' => 'meta_value_num', 'order' => 'DESC', 'post_status' => 'publish', 'ignore_sticky_posts' => true ) ) );
        if ($r->have_posts()) :
?>
 ...
 get_template_part( 'loop-templates/card', 'small-post' );
endwhile; ?>
...
?>
                        ...
                        <p>
                            <input class="checkbox" type="checkbox" <?php checked( $show_date ); ?> id="
...
function coopparty_register_custom_widgets() {
    register_widget( 'WP_Widget_Recent_Posts_w_thumbs' );
}
add_action( 'widgets_init', 'coopparty_register_custom_widgets' );
...
class taxonomy_dropdown_widget_plugin {
	...
	public function render_dropdown( $options, $id = false ) {
		...
		if ( is_array( $terms ) && ! empty( $terms ) ) {
			...
			// Start dropdown
			$output = '<select name="taxonomy_dropdown_widget_dropdown_' . esc_attr( $id ) . '" class="form-control taxonomy_dropdown_widget_dropdown" onchange="document.location.href=this.options[this.selectedIndex].value;"' . $css_id . '>' . "\r\n";
...
class taxonomy_dropdown_widget extends WP_Widget {
	...
	?>
                            ...
                            <p>
                                ...
                                <br />
                                <select class="form-control" name="<?php echo $this->get_field_name( 'taxonomy' ); ?>" id="<?php echo $this->get_field_id( 'taxonomy' ); ?>">
                                    <?php foreach ( $taxonomies as $tax ) : ?>
                                        <option value="<?php echo esc_attr( $tax->name ); ?>" <?php selected( $tax->name, $options['taxonomy'], true ); ?>>
                            ...
                            <p>
                                ...
                                <br />
                                <input type="text" name="<?php echo $this->get_field_name( 'title' ); ?>" class="widefat code" id="<?php echo $this->get_field_id( 'title' ); ?>" value="<?php echo esc_attr( $options['title'] ); ?>" /> </p>
                            <p>
                                <label for="<?php echo $this->get_field_id( 'select_name' ); ?>">
                                    <?php _e( 'Default dropdown item:' ); ?>
                                </label>
                                <br />
                                <input type="text" name="<?php echo $this->get_field_name( 'select_name' ); ?>" class="widefat code" id="<?php echo $this->get_field_id( 'select_name' ); ?>" value="<?php echo esc_attr( $options['select_name'] ); ?>" /> </p>
                            <h3><?php _e( 'Order' ); ?></h3>
                            <p>
                                <label>
                                    <?php _e( 'Order terms by:' ); ?>
                                </label>
                                <br />
                                ...
                                <label for="<?php echo $this->get_field_name( 'order_name' ); ?>">
                                    <?php _e( 'Name' ); ?>
                                </label>
                                <br />
                                ...
                                <label for="<?php echo $this->get_field_name( 'order_count' ); ?>">
                                    <?php _e( 'Post count' ); ?>
                                </label>
                            </p>
                            <p>
                                ...
                                <label for="<?php echo $this->get_field_name( 'order_asc' ); ?>">
                                    <?php _e( 'Ascending' ); ?>
                                </label>
                                <br />
                                ...
                                <label for="<?php echo $this->get_field_name( 'order_desc' ); ?>">
                                    <?php _e( 'Descending' ); ?>
                                </label>
                            </p>
                            ...
                            <p>
                                <label for="<?php echo $this->get_field_id( 'limit' ); ?>">
                                    <?php _e( 'Limit number of terms shown to:' ); ?>
                                </label>
                                <br />
                                <input type="text" name="<?php echo $this->get_field_name( 'limit' ); ?>" id="<?php echo $this->get_field_id( 'limit' ); ?>" value="<?php echo intval( $options['limit'] ); ?>" size="3" />
                                <br /> <span class="description"><small><?php _e( 'Enter <strong>0</strong> for no limit.' ); ?></small></span> </p>
                            <p>
                                <label for="<?php echo $this->get_field_id( 'max_name_length' ); ?>">
                                    <?php _e( 'Trim long term names to <em>x</em> characters:</label>' ); ?>
                                        <br />
                                        <input type="text" name="<?php echo $this->get_field_name( 'max_name_length' ); ?>" id="<?php echo $this->get_field_id( 'max_name_length' ); ?>" value="<?php echo intval( $options['max_name_length'] ); ?>" size="3" />
                                        <br /> <span class="description"><small><?php _e( 'Enter <strong>0</strong> to show full tag names.' ); ?></small></span> </p>
                            <p>
                                <label for="<?php echo $this->get_field_id( 'cutoff' ); ?>">
                                    <?php _e( 'Indicator that term names are trimmed:' ); ?>
                                </label>
                                <br />
                                <input type="text" name="<?php echo $this->get_field_name( 'cutoff' ); ?>" id="<?php echo $this->get_field_id( 'cutoff' ); ?>" value="<?php echo esc_attr( $options['cutoff'] ); ?>" size="3" />
                                <br /> <span class="description"><small><?php _e( 'Leave blank to use an elipsis (&hellip;).' ); ?></small></span> </p>
                            <p>
                                ...
                                <label for="<?php echo $this->get_field_id( 'hide_empty' ); ?>">
                                    <?php _e( 'Include terms that aren\'t assigned to any objects (empty terms).' ); ?>
                                </label>
                            </p>
                            <p>
                                ...
                                <label for="<?php echo $this->get_field_id( 'post_counts' ); ?>">
                                    <?php _e( 'Display object (post) counts after term names.' ); ?>
                                </label>
                            </p>
                            <h3><?php _e( 'Include/Exclude Terms' ); ?></h3>
                            <p>
                                <label>
                                    <?php _e( 'Include/exclude terms:' ); ?>
                                </label>
                                <br />
                                ...
                                <label for="<?php echo $this->get_field_id( 'include' ); ?>">
                                    <?php _e( 'Include only the term IDs listed below' ); ?>
                                </label>
                                <br />
                                ...
                                <label for="<?php echo $this->get_field_id( 'exclude' ); ?>">
                                    <?php _e( 'Exclude the term IDs listed below' ); ?>
                                </label>
                            </p>
                            <p>
                                <label for="<?php echo $this->get_field_id( 'incexc_ids' ); ?>">
                                    <?php _e( 'Term IDs to include/exclude based on above setting:' ); ?>
                                </label>
                                <br />
                                <input type="text" name="<?php echo $this->get_field_name( 'incexc_ids' ); ?>" class="widefat code" id="<?php echo $this->get_field_id( 'incexc_ids' ); ?>" value="<?php echo esc_attr( implode( ', ', $options['incexc_ids'] ) ); ?>" />
                                <br /> <span class="description"><small><?php _e( 'Enter comma-separated list of term IDs.' ); ?></small></span> </p>
                            <h3><?php _e( 'Advanced' ); ?></h3>
                            <p>
                                <label for="<?php echo $this->get_field_id( 'threshold' ); ?>">
                                    <?php _e( 'Show terms assigned to at least this many posts:' ); ?>
                                </label>
                                <br />
                                <input type="text" name="<?php echo $this->get_field_name( 'threshold' ); ?>" id="<?php echo $this->get_field_id( 'threshold' ); ?>" value="<?php echo intval( $options['threshold'] ); ?>" size="3" />
                                <br /> <span class="description"><small><?php _e( 'Set to <strong>0</strong> to display all terms matching the above criteria.' ); ?></small></span> </p>
                            <?php
	}
}
/**
...
class coop_cta_widget extends WP_Widget {
    

	function __construct() {
		// Instantiate the parent object
		parent::__construct( false, 'Call to action box' );
	}
    ...
    function widget($args, $instance) {	
        ...
        if(get_field('cta_text')):?>
                                    <?php echo $before_widget; ?>
                                        <?php if(get_field('cta_headline')){ echo '<h2>' . get_field('cta_headline') . '</h2>'; };
                if(get_field('cta_text')){ echo '<p class="text-center">' . get_field('cta_text') . '</p>'; };
                 if(get_field('cta_url')){ echo '<div class="text-center"><a href="' . get_field('cta_url') . '" class="btn-lg btn-wrap btn btn-block btn-' . get_field('btn_color') . '">' . get_field('btn_text') . '</a></div>'; };                
                ?>
                                            <?php echo $after_widget;
        endif;
    }
...
function coopparty_cta_widget() {
	register_widget( 'coop_cta_widget' );
}

add_action( 'widgets_init', 'coopparty_cta_widget' );
