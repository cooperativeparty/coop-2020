...
?>
    <article id="post-<?php the_ID(); ?>" <?php post_class( 'event py-1 py-2 border-bottom-1'); ?>>
        ...
        echo get_the_post_thumbnail($post->ID, 'square-sm', array( 'class' => 'card-img-top img-max' )  );
        echo '</a>';
        ?></div>
            <div class="col">
                <?php if ( $venue_details ) : ?>
                    ...
                    <?php endif; ?>
                        <?php the_title( sprintf( '<h5 class="card-title mt-2"><a class="text-microsite" href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h5>' ); ?>
